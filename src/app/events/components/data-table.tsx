...
function EventActions({ row, onRefresh }: EventActionProps) {
  ...
  const handleDelete = async (): Promise<void> => {
    const confirmed = await confirmDialog({
      ...
      description: "Esta ação excluirá o evento permanentemente.",
      confirmText: "Excluir",
      cancelText: "Cancelar",
    });
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-32">
        <DropdownMenuItem onClick={(e: React.MouseEvent) => e.stopPropagation()} asChild>
          <EventForm type="edit" initialData={row.original} onRefresh={onRefresh} />
        </DropdownMenuItem>
        <DropdownMenuSeparator />
...
const columns: ColumnDef<EventRow>[] = [
  {
    id: "drag",
    header: () => null,
    cell: ({ row }: { row: Row<EventRow> }) => <DragHandle id={row.original.id} />,
  },
  {
    accessorKey: "title",
    header: "Título",
    ...
    cell: ({ row }: { row: Row<EventRow> }) => (
      <div>
        <Badge variant="outline" className="text-muted-foreground px-1.5">
          {new Date(row.original.finishAt).toLocaleDateString("pt-BR")}
        </Badge>
      </div>
    ),
  },
  ...
  },
  {
    ...
    header: "Status",
    enableColumnFilter: true,
    filterFn: (row: Row<EventRow>, columnId: string, filterValue: string) => {
...
function DraggableRow({ row }: { row: Row<EventRow> }) {
...
export function DataTable({
  ...
  const [data, setData] = React.useState<EventRow[]>(() => initialData ?? []);
  ...
  const table = useReactTable({
    ...
    getRowId: (row: EventRow) => row.id.toString(),
...
function TableCellViewer({ item }: { item: EventRow }) {
    ...
    <Drawer direction={isMobile ? "bottom" : "right"}>
      ...
      <DrawerContent>
        <DrawerHeader className="gap-1">
          <DrawerTitle>Visualização do evento</DrawerTitle>
          <DrawerDescription>Visualizando dados de {item.title}</DrawerDescription>
        </DrawerHeader>
